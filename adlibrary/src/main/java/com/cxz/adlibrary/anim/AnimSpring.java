package com.cxz.adlibrary.anim;
...
public class AnimSpring {
    ...
    public void startCircleAnim(final int animType, final RelativeLayout animContainer) {
        double radius = Math.sqrt(DisplayUtil.screenHeightPx * DisplayUtil.screenHeightPx + DisplayUtil.screenWidthPx * DisplayUtil.screenWidthPx);
     ...
     * @param AdDialogUtil
     */
    public void stopAnim(int animType, final AdDialogUtil AdDialogUtil) {
        if (AdDialogUtil == null) {
            return;
        }

        if (animType == AdConstant.ANIM_STOP_TRANSPARENT) {
            AdDialogUtil.getAnimContainer().animate().alpha(0).setListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    AdDialogUtil.getAndroidContentView().removeView(AdDialogUtil.getRootView());
                    AdDialogUtil.setShowing(false);
                }
